    ...
    public class ICountryDtoValidator : AbstractValidator<ICountryDto>
    {
        ...
        public ICountryDtoValidator(ICountryRepository countryRepository)
        {
            ...
            RuleFor(p => p.ISO)
                ...
                {
                    bool tAreadyExists = await mCountryRepository.CheckISOExistsAsync(isoNum);
                    return !tAreadyExists;
            ...
            RuleFor(p => p.A3)
               ...
               {
                   bool tAreadyExists = await mCountryRepository.CheckA3CodeExistsAsync(a3);
                   return !tAreadyExists;
               }).WithMessage("{PropertyName} must be unique per country.}");
            ...
            RuleFor(p => p.A2)
               ...
               .WithMessage("{PropertyName} must be 2 characters in length.")
                ...
                {
                    bool tAreadyExists = await mCountryRepository.Check2CodeExistsAsync(a2);
                    return !tAreadyExists;
                }).WithMessage("{PropertyName} must be unique per country.}");
            ...
            RuleFor(p => p.DialingCode)
                ...
                {
                    bool tAreadyExists = await mCountryRepository.CheckDialingCodeExistsAsync(dialingCode);
                    return !tAreadyExists;
                }).WithMessage("{PropertyName} must be unique per country.}");
        }
    }
